        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="editOrder_pizzalForm" method="POST" action="{{ route('order_pizzas.update', $order_pizza->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="order_id" name="order_id">
                                ...
                                @foreach($orders as $order)
                                    <option value="{{ $order->id }}" {{ $order_pizza->order_id == $order->id ? 'selected' : '' }}>
                                        {{ $order->id }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="pizza_size_id" name="pizza_size_id">
                                ...
                                @foreach($pizza_size as $size)
                                    <option value="{{ $size->id }}" {{ $order_pizza->pizza_size_id == $size->id ? 'selected' : '' }}>
                                        {{ $size->size }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="quantity" name="quantity" placeholder="Cantidad" value="{{ $order_pizza->quantity }}">
                        </div>
...
<script>
    $(document).ready(function() {
        $('#editOrder_pizzalForm').on('submit', function(event) {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Orden de Pizza editado con éxito',
                showConfirmButton: false,
                timer: 1500
            }).then(function() {
                // Si todo es correcto, proceder con el envío del formulario
                $('#editOrder_pizzalForm').unbind('submit').submit();
            });
        });
    });
</script>