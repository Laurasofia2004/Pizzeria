        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="editPizza_raw_materialForm" method="POST" action="{{ route('pizza_raw_materials.update', $pizza_raw_material->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="pizza_id" name="pizza_id">
                                ...
                                <option value="{{ $pizza->id }}"
                                    {{ $pizza_raw_material->pizza_id == $pizza->id ? 'selected' : '' }}>
                                    {{ $pizza->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="raw_material_id" name="raw_material_id">
                                ...
                                <option value="{{ $raw_material->id }}"
                                    {{ $pizza_raw_material->raw_material_id == $raw_material->id ? 'selected' : '' }}>
                                    {{ $raw_material->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="quantity" name="quantity" placeholder="Cantidad" value="{{ $pizza_raw_material->quantity }}">
                        </div>
...
<script>
    $(document).ready(function() {
        $('#editPizza_raw_materialForm').on('submit', function(event) {
            ...
            Swal.fire({
                ...
                $('#editPizza_raw_materialForm').unbind('submit').submit();
            });
        });
    });
</script>